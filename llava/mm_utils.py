...
def process_image(
    ...
    if isinstance(image_file, str):
        ...
        if 'pdf' not in image_file:
            image = Image.open(image_file).convert("RGB")
        else:
            image = pdf_to_images_high_quality(image_file)
    else:
        # image is stored in bytearray
        image = image_file
        image = image.convert("RGB")

    if hasattr(processor, 'name') and processor.name == "sam_clip_processor":
        # print(">>>using sam_clip_processor")
