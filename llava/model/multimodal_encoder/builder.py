...
from .sam_clip_encoder import SAMCLIPVisionTower, SAMCLIPVisionTowerS2
...
def build_vision_tower(model_name_or_path: str, config: PretrainedConfig) -> PreTrainedModel:
    ...
    if config.resume_path and "radio" not in model_name_or_path and "sam_clip" not in model_name_or_path:
    ...
    elif "sam_clip" in vision_tower_name.lower():
        if use_s2:
            vision_tower = SAMCLIPVisionTowerS2(model_name_or_path, config)
        else:
            vision_tower = SAMCLIPVisionTower(model_name_or_path, config)
