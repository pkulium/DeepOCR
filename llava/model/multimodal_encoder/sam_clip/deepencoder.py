...
class CLIPVisionEmbeddings(nn.Module):
    ...
    def forward(self, pixel_values, patch_embeds):
        ...
        if self.position_ids.device != device:
...
def build_clip_l(ckpt=None):
    ...
    if ckpt is not None:
        print(">>>loading CLIP checkpoint:", ckpt)
        ...
        filtered_state_dict = {k[len(prefix):]: v for k, v in state_dict.items() if k.startswith(prefix)}

        # Load with strict=False to handle missing buffers gracefully
        missing_keys, unexpected_keys = model.load_state_dict(filtered_state_dict, strict=False)
        
        # Log any issues for debugging
        if missing_keys:
            print(f"Missing keys (will be initialized with default values): {missing_keys}")
        if unexpected_keys:
            print(f"Unexpected keys (ignored): {unexpected_keys}")
        
        print(f"Successfully loaded CLIP checkpoint: {ckpt}")
    
    # CRITICAL FIX: Ensure position_ids buffer is properly initialized and on correct device
    # This should be done AFTER loading the checkpoint to ensure it's on the same device as other parameters
    if hasattr(model.embeddings, 'position_ids'):
        # Get the device from another parameter in the model
        device = next(model.parameters()).device
        # Recreate position_ids buffer on the correct device
        model.embeddings.register_buffer(
            "position_ids", 
            torch.arange(model.embeddings.num_positions).expand((1, -1)).to(device)
        )
        print(f"Reinitialized position_ids buffer on device: {device}")
...
def _build_sam(
    ...
    if checkpoint is not None:
        ...
        # Fix: Use the correct prefix "model.sam_model." (17 characters)
        prefix = "model.sam_model."
        filtered_state_dict = {k[len(prefix):]: v for k, v in state_dict.items() if k.startswith(prefix)}
        
        # Load the filtered state dict
        image_encoder.load_state_dict(filtered_state_dict, strict=True)
        print(f"Successfully loaded sam checkpoint: {checkpoint}")
        print(checkpoint)
    return image_encoder