...
class LlavaMetaModel(ABC):
    ...
    def encode_images(self, images, block_sizes: Optional[Optional[Tuple[int, ...]]] = None):
        ...
        if getattr(self.config, "dynamic_s2", False):
            ...
            print(">>>using vision tower then projector")
            image_features = self.get_vision_tower()(images)
            image_features = self.get_mm_projector()(image_features)
...
class LlavaMetaForCausalLM(ABC):
    def _embed(
        ...
        print(">>>using __embed_media_tokens")
    ...
    def __embed_media_tokens(
        ...
        for name in media:
            if self.training:
                ...
                # for tensor in media.get(name, []):
                #     if not isinstance(tensor, dict):
                #         continue
                #     for key in tensor:
                #         tensor[key] = tensor[key].to(self.device)

                info = [{"shape": tensor.shape, "dtype": tensor.dtype} if not isinstance(tensor, dict) else {"shape": tensor['pixel_values'].shape, "dtype": tensor['pixel_values'].dtype} for tensor in media.get(name, [])]
                ...
                if media.get(name) is None or len(media[name]) == 0:
                    ...
                    continue
            # print(">>>using sam_clip_vision_tower with")
            ...
            if name == "image":
                if len(media["image"]) == 1 and self.config.image_aspect_ratio in ["dynamic", "dynamic_s2"]:
                    ...
                    if self.config.image_aspect_ratio == "dynamic":
                        if isinstance(media["image"][0], torch.Tensor):
                            images = process_image(media["image"][0], self.config, None, enable_dynamic_res=True).half()
                            conversation[0]["value"] = conversation[0]["value"].replace(
                                DEFAULT_IMAGE_TOKEN, f"{DEFAULT_IMAGE_TOKEN}\n" * images.shape[0]
                            )
                        else:
                            images = process_image(media["image"][0], self.config, None, enable_dynamic_res=True)
                            for key in images:
                                images[key] = images[key].to(dtype=torch.bfloat16)
                            conversation[0]["value"] = conversation[0]["value"].replace(
